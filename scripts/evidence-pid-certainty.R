...
pid_certainty <- evidence |> 
  ...
  nest() |>
  mutate(
    model = map(data, ~lm(not_sure ~ ICE_Video * pid, data = .)),
...
ggplot(pid_certainty, aes(x = decrease, y = reorder(question, decrease), color = pid)) +
  ...
  labs(
    title = "Effect of ICE video on attitude certainty varies slightly by party",
    subtitle = "Predicted probability of respondents saying 'Not sure', from OLS interacted by partisanship",
    x = "",
    y = NULL,
    color = NULL) +
  theme_minimal() +
